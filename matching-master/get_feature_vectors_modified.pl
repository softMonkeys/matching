...
my $keywords_file = 'get_keywords_pos.tra.out';
my $keyword_count = 500;
my %keyword = ();
my @keyword_list = ();

my $keyword_intersection_file = 'get_keywords_intersection_freq.tra.out';
my %keyword_intersection = ();
my @keyword_intersection_list = ();

my $keybigrams_file = 'get_key_bigrams_pos.out';
...
foreach my $f (@files) {
    ...
    my $in_job = 0; # 1 if we are in the job description
	...
	if ($field eq 'stages') {
		...
		$app_labels{$app_number} = '0';#One stage means applicant is new
	    }
	    elsif ($line =~ /2/) {
		$app_labels{$app_number} = '-1';#Two stages means applicant went from new to rejected
	    }
	    else {
		$app_labels{$app_number} = '+1';
	    }
	...
	if ($in_app) {
		...
		$application .= "$line " unless $field =~ 'stages';
	    }
	    #print "$line\n";
	}

        # Check if the current application is ending.
	#print "$brace_change $bracket_change $brace_level $new_app_brace_level $bracket_level $new_app_bracket_level\n";
	...
	if ($field eq 'applications') {
	    ...
	    #print "$new_app_brace_level\n";
	    #print "$new_app_bracket_level\n";
	    next;
	}
	...
	elsif($in_exp && $field eq 'description')
	{
	    if($from_date eq '')
	    {
		$from_date = 0;
	...
	elsif ($field eq 'job_description')
	{
	   $in_job = 1;
	}
	elsif ($in_job == 1)
        {
            #Right now, we can only handle english jobs; if that changes, turn the 'en' into a variable
	    my $l = $line;
	    $l =~ s/\\n/ /g;
	    $l = lc $l;
	    $l =~ s/[^\w\d\s]/ /g;
